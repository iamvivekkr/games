...
router.post("/aadharcard", Auth, upload.fields([{ name: 'front', maxCount: 1 }, { name: 'back', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        ...
        if (!validateAadhaar(Number)) {
            return res.send({ status:false, msg: 'Not a valid Aadhar Number' })
        }
        if (data && data.verified === "verified") {
            return res.send({ status:false, msg: 'Your kyc is already verifyed. Please connect with admin' })
        }
        ...
        if (data && data.verified === "pending") {
            return res.send({ status: false, msg: 'Kyc already submited. Please wait for admin verify' })
        }
...
router.delete("/aadharcard/:id", Auth, async (req, res) => {
    ...
    const dir = data?data.front:null;
    const dir1=data?data.back:null;
...
try {
  ...
  var user = await User.findOneAndUpdate({_id:data.User},updateDoc);
      var statuskyc = await AadharCard.updateMany({ User: data.User },{
        $set: {
            verified: 'reject',
        },
      });
  if(user.verified != 'unverified'){     
          var addActivity =  new activity({
            User_id:user._id,
            Req_type: "kyc",
            txn_msg: user.Name+" kyc force delete by admin using link.",
