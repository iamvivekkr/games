...
router.get("/temp/deposite/:id", Auth, async (req, res) => {
  ...
  await Transaction.find({ User_id: req.params.id })
    .sort({ updatedAt: -1 })
    .limit(PAGE_SIZE)
    .skip(PAGE_SIZE * page)
    .then((ress) => {
...
router.get("/temp/deposit/pending", Auth, async (req, res) => {
  try {
    let data = await Transaction.find({
      Req_type: "deposit",
      status: "pending",
    }).populate("User_id", "_id Name Phone");
    data = data.map((txn) => {
      const txnObj = txn.toObject();
      txnObj.user = txnObj.User_id;
      delete txnObj.User_id;
      return txnObj;
    });
  ...
  try {
    const data = await Temp.find({
      $and: [
        ...
        {
          $or: [{ status: "success" }, { status: "reject" }],
        },
      ],
    }).populate("user");
    res.status(200).send(data);
...
router.patch("/temp/withdraw/reject/:id", Auth, async (req, res) => {
  try {
    ...
    if (user.withdraw_holdbalance > 0) {
      ...
      res.send({
        message: "Invalid request, withdraw_holdbalance low",
        error: true,
      });
    }
  } catch (e) {
