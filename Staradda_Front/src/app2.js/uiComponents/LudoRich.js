...
export default function LudoRich({ walletUpdate }) {
  ...
  const ChallengeCreate = (e) => {
    if (Game_Ammount < 500) {
      alert("Amount range between 500 -10000");
      return false;
    }
    setIsloading(true);
    const access_token = localStorage.getItem("token");
    ...
    axios
      ...
      .then((res) => {
        ...
        // console.log(res?.data?.data?.Game_Ammount,Game_Ammount)
    ...
    io.on("resultUpdateReq", (data) => {
      ...
      data.forEach(function (ele) {
        if (ele.Created_by && ele.Accepetd_By)
          if (
            ele.Created_by._id == userID.current ||
            (ele.Accepetd_By._id == userID.current && ele.gameAmount >= 500)
          ) {
            owenedRunning.push(ele);
          } else {
            remainingRunning.push(ele);
  ...
  const AcceptChallang = (id) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        // console.log(res.data.Accepetd_By)
        str = 0;
        if (res.data.status === false) {
          Swal.fire({
            title: res.data.msg,
          ...
          {JSON.parse(localStorage.getItem("sitSetting")) == "undefined" ? (
            ...
            <>
              <span className={`${css.cxy} ${css.battleInputHeader} mt-4`}>
                Create a Battle between 500-10000
              </span>
